...
import io.streamnative.pulsar.handlers.amqp.*;
...
public class PersistentExchange extends AbstractAmqpExchange {
    ...
    public PersistentExchange(String exchangeName, Type type, PersistentTopic persistentTopic, boolean autoDelete) {
        super(exchangeName, type, new CopyOnWriteArraySet<>(), new ConcurrentHashMap<>(), true, autoDelete);
        ...
        if (messageReplicator == null) {
            messageReplicator = new AmqpExchangeReplicator(this) {
                ...
                public CompletableFuture<Void> readProcess(Entry entry) {
                    ...
                    //默认exchange
                    if (exchangeName.startsWith(AbstractAmqpExchange.DEFAULT_EXCHANGE_DURABLE)){
                        AmqpQueue amqpQueue = queueMap.get(routingKey);
                        if (!Objects.isNull(amqpQueue)){
                            routeFutureList.add(amqpQueue.getRouter(exchangeName).routingMessage(
                                    entry.getLedgerId(), entry.getEntryId(),
                                    routingKey,
                                    props
                            ));
                        }
//                        log.info("PersistentExchange default exchangeName: [{}] bindQueueSize: [{}] timeConsumer: [{}]ms", exchangeName, queues.size(), System.currentTimeMillis() - startTime);
                    }else {
                        for (AmqpQueue queue : queues) {
                            log.debug("routing msg from exchange={} to queue={} by key={} for ledgerId={}, entryId={}, props={}.",
                                    exchangeName, queue.getName(), props.getOrDefault(MessageConvertUtils.PROP_ROUTING_KEY, "").toString(),
                                    entry.getLedgerId(), entry.getEntryId(), props);
//                            AmqpMessageRouter router = queue.getRouter(exchangeName);
//                        log.info("exchange:{}---router type:{}---router bindKey:{}---router queue:{}",router.getExchange(),router.getType(),router.getBindingKey(),router.getQueue());
...
//                        log.info("PersistentExchange else exchangeName: [{}] bindQueueSize: [{}] timeConsumer: [{}]ms", exchangeName, queues.size(), System.currentTimeMillis() - startTime);
                    }
    ...
    public void addQueue(AmqpQueue queue) {
        // log.info("start to add queue: " + queue.getName() + " to exchange: " + this.exchangeName + ", queueSize=" + queues.size());
        queueMap.put(queue.getName(), queue);
        queues.add(queue);
        // log.info("end to add queue: " + queue.getName() + " to exchange: " + this.exchangeName + ", queueSize=" + queues.size());
        updateExchangeProperties();
    ...
    //             properties.put(QUEUES, jsonMapper.writeValueAsString(queueNameSet));
    //         }

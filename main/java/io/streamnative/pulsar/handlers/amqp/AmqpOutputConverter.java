...
public class AmqpOutputConverter {
    ...
    public long writeDeliver(final AmqpMessageData message, int channelId,
        ...
        String msgId = message.getContentHeaderBody().getProperties().getMessageIdAsString();
        String uuid = message.getContentHeaderBody().getProperties().getAppIdAsString();
        long startTime = message.getContentHeaderBody().getProperties().getTimestamp();
        long processTime = startTime == 0 ? 0L : (System.currentTimeMillis() - startTime);
        log.info("[{}] writeDeliver: [{}], deliveryTag: [{}], consumerTag: [{}], isRedelivered [{}], msgId: [{}], processTime: [{}]ms, uuid: [{}], remoteAddress: [{}] localAddress: [{}]"
                , connection.ctx.name(), str(message.getContentBody().getPayload()), deliveryTag, consumerTag, isRedelivered, msgId, processTime, uuid, connection.getRemoteAddress(), connection.getLocalAddress());
        if (processTime > 1000){
            log.info("processTime too long!!! processTime: [{}]", processTime);
        }

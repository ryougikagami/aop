...
public abstract class AmqpExchangeReplicator implements AsyncCallbacks.ReadEntriesCallback,
    ...
    private static final int replicatorQueueSize = 100;
    ...
    protected AmqpExchangeReplicator(PersistentExchange persistentExchange) {
        ...
        log.info("replicatorName---------"+this.name);
    }

    public void startReplicate() {
        if (STATE_UPDATER.get(AmqpExchangeReplicator.this).equals(State.Stopping)) {
    ...
    public void readEntriesComplete(List<Entry> list, Object o) {
        ...
        // log.info"{} Read entries complete. Entries size: {}", name, list.size());

        long startTime = System.currentTimeMillis();
        for (Entry entry : list) {
            try {
                PENDING_SIZE_UPDATER.incrementAndGet(this);
//                // log.infolist.size()+"-----------"+ JSON.toJSONString(list)+"-----------"+entry);
                ...
                log.warn("Route message failed."+list.size()+entry.toString(), e);
            }
        }
        log.info("readEntriesComplete exchange: [{}] entrySize: [{}] entryConsumeTime: [{}]ms", persistentExchange.exchangeName, list.size(), System.currentTimeMillis() - startTime);

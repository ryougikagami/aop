...
public class AmqpProtocolHandler implements ProtocolHandler {
    ...
    public static final String LISTENER_PATTEN = "^(amqp+ssl)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-0-9+]";
    ...
    public void start(BrokerService service) {
        ...
        try {
            service.getPulsar().getAdminClient().namespaces().createNamespace("public/vhost1",3);
        ...
        if (amqpConfig.isAmqpProxyEnable()) {
            ...
            proxyConfig.setAmqpEnableTls(amqpConfig.isAmqpEnableTls());
            proxyConfig.setAmqpServerKeystore(amqpConfig.getAmqpServerKeystore());
            proxyConfig.setAmqpServerKey(amqpConfig.getAmqpServerKey());
            proxyConfig.setZookeeperServers(amqpConfig.getZookeeperServers());
    ...
    public Map<InetSocketAddress, ChannelInitializer<SocketChannel>> newChannelInitializers() {
        ...
        try {
            ...
            for (String listener: parts) {
                // log.info"amqpprotocol"+JSON.toJSONString(listener));
                if (listener.startsWith(PLAINTEXT_PREFIX)||listener.startsWith(SSL_PREFIX)) {
                    builder.put(
